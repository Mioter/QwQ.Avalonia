...
public static class TaskManagerExample
{
    ...
    private static async Task BasicTaskExample()
    {
        ...
        var result = await TaskManager.CreateTask(() =>
        {
        ...
        Console.WriteLine($"任务执行结果: {result.Result}, 耗时: {result.ExecutionTime.TotalMilliseconds}ms");
    }
    ...
    private static async Task TaskWithControllerExample()
    {
        ...
        var result = await TaskManager.CreateTask(token =>
        {
            for (int i = 1; i <= 5; i++)
            {
        ...
        Console.WriteLine($"控制器任务状态: {result.FinalState}, 耗时: {result.ExecutionTime.TotalSeconds:F1}秒");
    }
    ...
    private static async Task AsyncTaskExample()
    {
        ...
        var result = await TaskManager.CreateTask(async () =>
        {
            Console.WriteLine("开始异步网络请求模拟...");
            await Task.Delay(1500); // 模拟网络请求
            return $"数据获取完成 - {DateTime.Now:HH:mm:ss}"; 
        })
        .SetTimeout(3000, elapsed => Console.WriteLine($"请求超时，已用时: {elapsed.TotalSeconds:F1}秒"))
    ...
    private static async Task ErrorHandlingExample()
    {
        ...
        Console.WriteLine($"错误任务状态: {result.FinalState}, 异常类型: {result.Exception?.GetType().Name}");
    }
    ...
    private static async Task TimeoutExample()
    {
        ...
        .SetTimeout(2000, elapsed => Console.WriteLine($"[超时处理器] 任务超时，已运行 {elapsed.TotalSeconds:F1} 秒"))
        .SetErrorHandler(ex => Console.WriteLine($"任务异常: {ex.Message}"))
        .RunAsync();
        
        Console.WriteLine($"超时任务状态: {result.FinalState}, 实际耗时: {result.ExecutionTime.TotalSeconds:F1}秒");
    }
    ...
    private static async Task BackgroundVsForegroundExample()
    {
        ...
        // 后台任务
        var backgroundTask = TaskManager.CreateTask(() =>
        {
        ...
        // 前台任务
        var foregroundTask = TaskManager.CreateTask(() =>
        {
        ...
        Console.WriteLine($"两个任务都已完成，后台任务: {results[0].IsSuccess}, 前台任务: {results[1].IsSuccess}");
    }
    ...
    public static async Task ChainOrderTest()
    {
        ...
        // 测试不同的链式调用顺序
        var result = await TaskManager.CreateTask(() => "测试结果")
    ...
    private static async Task MultiProjectParallelExample()
    {
        ...
        // 创建并行执行的多项目任务
        var result = await TaskManager.CreateMultiTask(
            items: items,
            itemWork: async item =>
            {
        ...
        .SetCompleteCallback(results => Console.WriteLine($"所有项目处理完成，共 {results.Count()} 个结果"))
        .RunAsync();
